...
- chat: 问候、感谢、身份介绍、纯闲聊、不需要查任何公司内部规范的普通概念问答
- task: 需要读写文件、执行命令、安装包、搜索网络、查询系统状态、分析日志、**查询公司内部规范/配置/命名规则**、产出具体成果的请求

注意：如果问题涉及公司内部才知道的信息（如内部端口分配、服务器命名规则、告警流程等），必须归为 task。
