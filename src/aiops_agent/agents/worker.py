...
AGENT_DEF = {
    ...
    "system_prompt_template": (
        ...
        "1. 按 TODO 列表依次执行运维操作\n"
        "2. 使用可用工具逐一完成每个 TODO\n"
        "3. 每个 TODO 完成后调用 update_todo 工具标记完成状态\n"
        "4. 全部完成后给出简洁的最终报告\n"
        ...
        "- 当需要参考公司内部规范时（端口分配、日志目录、告警阈值、命名规则等），"
        "使用 retrieve_knowledge 工具查询知识库"
    ),
    "tools": ["shell", "read_file", "write_file", "retrieve_knowledge", "update_todo"],
}
